...
public:
    ...
    array(int rsv): std::vector<T>(static_cast<std::vector<T>::size_type>(rsv))
    {
...
class hash : public /*stdext::hash_map<T, U, hash_functor>*/ std::unordered_map<T, U,hash_functor>
{
public:
...
// // lfl_string is a subset of std::string, for the most part
// class lfl_string : public std::string
// {
// public:
// 	lfl_string(const char* str) : std::string(str) {}
// 	lfl_string() : std::string() {}
// 	lfl_string(const lfl_string& str) : std::string(str) {}
...
// template<class U>
// class string_hash : public hash<lfl_string, U, std::hash<std::string> >
// {
// };
...
public:
    ...
    {
        assert(index >= 0 && index < m_size); return m_buffer[index];
    }
...
public:
    ...
    lfl_string(const char* buf, int buflen)
    {
        m_union.m_local.m_bufsize_minus_length = sizeof(m_union.m_local.m_buffer);
        m_union.m_local.m_buffer[0] = 0;
    ...
    lfl_string(const lfl_string& str)
    {
        m_union.m_local.m_bufsize_minus_length = sizeof(m_union.m_local.m_buffer);
        m_union.m_local.m_buffer[0] = 0;
    ...
    lfl_string(const uint32* wide_char_str)
    {
        m_union.m_local.m_bufsize_minus_length = sizeof(m_union.m_local.m_buffer);
        m_union.m_local.m_buffer[0] = 0;
    ...
    lfl_string(const uint16* wide_char_str)
    {
        m_union.m_local.m_bufsize_minus_length = sizeof(m_union.m_local.m_buffer);
        m_union.m_local.m_buffer[0] = 0;
    ...
    exported_module void	operator=(const lfl_string& str)
    {
        if (this != &str) {
            resize(str.size());
            memcpy(get_buffer(), str.get_buffer(), size() + 1);
    ...
    exported_module bool	operator==(const lfl_string& str) const
    {
        if (size() != str.size()) {
            return false;
        }
    ...
    {
        return (*(const_cast<lfl_string*>(this)))[index];
    }
    ...
    exported_module lfl_string	operator+(const lfl_string& str) const
    {
        lfl_string	new_string(*this);
        new_string += str;
        return new_string;
    }

    exported_module lfl_string	operator+(const char* str) const
    ...
    exported_module static void	encode_utf8_from_uint32(lfl_string* result, const uint32* wstr);
    exported_module static void	encode_utf8_from_uint16(lfl_string* result, const uint16* wstr);
    exported_module static void	encode_utf8_from_wchar(lfl_string* result, const wchar_t* wstr);
    ...
    exported_module lfl_string utf8_to_upper() const;

    // Return the string in this container as all lower case letters
    exported_module lfl_string utf8_to_lower() const;
    ...
    // Returns a lfl_string that's a substring of this.  start and
    ...
    exported_module lfl_string	utf8_substring(int start, int end) const;
...
private:
    ...
    {
        return const_cast<lfl_string*>(this)->get_buffer();
    }
    ...
    // The idea here is that lfl_string is an N-byte structure,
...
template<class U>
class string_hash : public hash<lfl_string, U, string_hash_functor<lfl_string> >
{
};
...
lfl_string string_printf(const char* fmt, ...)
#ifdef __GNUC__
