...
void lfl_string::append_wide_char(uint16 c)
{
	char buf[8];
	int index = 0;
...
void lfl_string::append_wide_char(uint32 c)
{
	char buf[8];
	int index = 0;
...
void	lfl_string::resize(int new_size)
{
	assert(new_size >= 0);
...
/*static*/ void	encode_utf8_from_wchar_generic(lfl_string* result, const char_type* wstr)
{
...
void lfl_string::encode_utf8_from_uint32(lfl_string* result, const uint32* wstr)
{
...
void lfl_string::encode_utf8_from_uint16(lfl_string* result, const uint16* wstr)
{
	encode_utf8_from_wchar_generic<uint16>(result, wstr);
}

void lfl_string::encode_utf8_from_wchar(lfl_string* result, const wchar_t* wstr)
{
...
/*static*/ int	lfl_string::stricmp(const char* a, const char* b)
{
#ifdef _WIN32
	return _stricmp(a, b);
#else
...
uint32	lfl_string::utf8_char_at(int index) const
{
...
lfl_string	lfl_string::utf8_to_upper() const
{
	const char*	buf = get_buffer();
	lfl_string str;
	for (;;)
...
lfl_string	lfl_string::utf8_to_lower() const
{
	const char*	buf = get_buffer();
	lfl_string str;
	for (;;)
...
/*static*/ int	lfl_string::utf8_char_count(const char* buf, int buflen)
{
	const char*	p = buf;
	int	length = 0;
...
lfl_string	lfl_string::utf8_substring(int start, int end) const
{
	assert(start <= end);
	...
	return lfl_string(start_pointer, int(end_pointer - start_pointer));
}
...
void test_unicode()
{
	...
	lfl_string::encode_utf8_from_wchar(&a, L"19 character string");
	assert(a.length() == 19);
...
{
#if 1
	printf("sizeof(lfl_string) == %d\n", sizeof(lfl_string));

	array<lfl_string>	storage;
	storage.resize(2);

	lfl_string&	a = storage[0];
	lfl_string&	b = storage[1];
	a = "test1";
	...
	c.resize(2);
	assert(c == "te");
	assert(c == lfl_string("te"));

	assert(lfl_string("fourscore and sevent") == "fourscore and sevent");
	...
	// Test operator<(const lfl_string&)
	a = "abc";
	assert(a < lfl_string("def"));
	assert(a < lfl_string("abd"));
	assert(!(a < lfl_string("aab")));
	assert(!(a < lfl_string("aa")));
	assert(!(a < lfl_string("aabz")));
	assert(a < lfl_string("abcd"));
