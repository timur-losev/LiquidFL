...
namespace gameswf
{
    ...
    bool as_value::find_property( const lfl_string& name, as_value* val)
    {
        switch (m_type)
        {
        default:
    ...
    bool as_value::find_property_owner(const lfl_string& name, as_value* val)
    {
        as_value dummy;
    ...
    void	as_value::set_string(const lfl_string& str)
    {
        drop_refs();
        m_type = STRING;
        m_string = str; 
    ...
    as_value::as_value( const lfl_string& str ):
        m_type(STRING),
        m_string(str),
        m_flags(0)
    {
        ...
        m_flags(0)
    {
        ...
        lfl_string::encode_utf8_from_wchar(&m_string, wstr);
    }
    ...
    void	as_property::set(as_object* target, const as_value& val)
    {
        if (target != NULL)
        {
            ...
            if (m_setter != NULL)
            {
                ...
                (*m_setter.get_ptr())(function_call(NULL, tar.get_ptr(),	&env, 1, env.get_top_index()));
            }
        }
    }
    ...
    void as_property::get(as_object* target, as_value* val) const
    {
        ...
        {
            gc_ptr<as_object> tar = target;
            (*m_getter.get_ptr())(function_call(val, tar.get_ptr(), &env, 0,	0));
        }
    }
    ...
    void as_property::get(const as_value& primitive, as_value* val) const
    {
        if (m_getter != NULL)
        {
            (*m_getter.get_ptr())(function_call(val, primitive, NULL, 0,	0));
        }
    }
