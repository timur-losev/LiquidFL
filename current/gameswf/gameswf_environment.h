...
namespace gameswf
{
	...
	exported_module lfl_string get_full_url(const lfl_string& workdir, const char* url);
...
//				(*this)[m_stack_size] = as_value(val);
	...
	struct as_environment : public vm_stack
	{
		...
		struct frame_slot
		{
			...
			frame_slot() {}
			frame_slot(const lfl_string& name, const as_value& val) : m_name(name), m_value(val) {}
		};
		...
		as_value	get_variable(const lfl_string& varname, const array<with_stack_entry>& with_stack) const;
		// no path stuff:
		as_value	get_variable_raw(const lfl_string& varname, const array<with_stack_entry>& with_stack) const;

		void	set_variable(const lfl_string& path, const as_value& val, const array<with_stack_entry>& with_stack);
		// no path stuff:
		void	set_variable_raw(const lfl_string& path, const as_value& val, const array<with_stack_entry>& with_stack);

		void	set_local(const lfl_string& varname, const as_value& val);
		void	add_local(const lfl_string& varname, const as_value& val);	// when you know it doesn't exist.
		void	declare_local(const lfl_string& varname);	// Declare varname; undefined unless it already exists.
		...
		static bool	parse_path(const lfl_string& var_path, lfl_string* path, lfl_string* var);

		// Internal.
		int	find_local(const lfl_string& varname, bool ignore_barrier) const;
	...
	struct function_call
	{
		...
		function_call(as_value* res_in, const as_value& this_in, as_environment* env_in, int nargs_in, int first_in) :
			result(res_in),
			this_value(this_in),
			env(env_in),
			nargs(nargs_in),
