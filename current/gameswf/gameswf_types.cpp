...
//	}
	...
	void	matrix::read(lfl_stream* in)
	...
	void	cxform::read_rgb(lfl_stream* in)
	{
		in->align();
	...
	void	cxform::read_rgba(lfl_stream* in)
	{
		in->align();
	...
	void	rgba::read(lfl_stream* in, int tag_type)
	{
		if (tag_type <= 22)
		{
			read_rgb(in);
	...
	void	rgba::read_rgba(lfl_stream* in)
	{
		read_rgb(in);
		m_a = in->read_u8();
	}

	void	rgba::read_rgb(lfl_stream* in)
	{
		m_r = in->read_u8();
		m_g = in->read_u8();
		m_b = in->read_u8();
