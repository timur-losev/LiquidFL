...
namespace gameswf
{
	...
	static void	dump_tag_bytes(lfl_stream* in)
	...
	void	movie_def_impl::export_resource(const lfl_string& symbol, character_def* res)
	...
	character_def*	movie_def_impl::get_exported_resource(const lfl_string& symbol)
	...
	void	movie_def_impl::add_import(const lfl_string& source_url, int id, const lfl_string& symbol)
	...
	void	movie_def_impl::add_scene(int scene, const lfl_string& frame_name)
	{
		m_scene.add(scene, frame_name);
	};

	void	movie_def_impl::add_frame_label(int frame, const lfl_string& frame_name)
	{
	...
	// Read a .SWF movie.
	{
		...
		m_file_stream = new lfl_stream(in);

		m_frame_size.read(m_file_stream);
		m_frame_rate = m_file_stream->read_u16() / 256.0f;

		set_frame_count(m_file_stream->read_u16());
	...
	void	movie_def_impl::read_tags()
	{
		...
		while ((Uint32) m_file_stream->get_position() < m_file_end_pos && get_break_loading() == false)
		{
			int	tag_type = m_file_stream->open_tag();
			...
			if (s_tag_loaders.get(tag_type, &lf))
			{
				...
				(*lf)(m_file_stream, tag_type, this);
			...
			else
			{
				...
				IF_VERBOSE_PARSE(dump_tag_bytes(m_file_stream));
			}

			m_file_stream->close_tag();

			if (tag_type == 0)
			{
				if ((unsigned int) m_file_stream->get_position() != m_file_end_pos)
				{
			...
			m_loaded_length = m_file_stream->get_position();
		}
