...
namespace gameswf
{
	...
	{
		// reads FILTERLIST
		const int filters = in->read_u8();
		for (int i = 0; i < filters; ++i)
		{
			int filter_id = in->read_u8();
			switch (filter_id)
			{
				case FILTER_DropShadowFilter:
				{
				...
				case FILTER_GradientGlowFilter:
				{
				...
				case FILTER_GradientBevelFilter:
				{
					int num_colors = in->read_u8();		// Number of colors in the gradient
					for (int i = 0; i < num_colors; i++)
					{
