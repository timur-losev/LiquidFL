...
namespace gameswf
{
	...
	void	as_object_addproperty(const function_call& fn)
	{
		if (fn.nargs == 3)
		{
			assert(fn.this_ptr);
	...
	void	as_object_registerclass(const function_call& fn)
	{
		fn.result->set_bool(false);
	...
	void	as_object_hasownproperty(const function_call& fn)
	{
		if (fn.nargs == 1)
		{
			assert(fn.this_ptr);
	...
	void	as_object_watch(const function_call& fn)
	{
		bool ret = false;
		if (fn.nargs >= 2)
		{
	...
	// false otherwise.
	void	as_object_unwatch(const function_call& fn)
	{
		bool ret = false;
		if (fn.nargs == 1)
		{
	...
	// for debugging
	void	as_object_dump(const function_call& fn)
	{
		if (fn.this_ptr)
		{
			lfl_string tabs;
	...
	void	as_global_object_ctor(const function_call& fn)
	...
	// flash9
	void	as_object_add_event_listener(const function_call& fn)
	...
	void as_object::call_watcher(const lfl_stringi& name, const as_value& old_val, as_value* new_val)
	{
		if (m_watch)
		{
			...
			if (watch.m_func)
			{
				...
				(*watch.m_func)(function_call(new_val, this, &env, 4, env.get_top_index()));
			}
		}
	}
	...
	bool	as_object::find_property( const lfl_stringi & name, as_value * val )
	{
		...
		if(m_instance.get_ptr() != NULL)
		{
			...
			for (int i = 0; i < m_instance->m_trait.size(); i++)
			{
				...
				const lfl_stringi& traits_name = m_instance->m_abc->get_multiname(ti->m_name);
...
//		}
	...
	bool as_object::watch(const lfl_string& name, as_function* callback,
	...
	bool as_object::unwatch(const lfl_string& name)
	{
		if (m_watch)
		{
			as_watch watch;
	...
	void as_object::dump(lfl_string& tabs)
	...
	// Find the object referenced by the given target.
	{
		...
		if (slash)
		{
			lfl_string name(path.c_str(), int(slash - path.c_str()));
			get_member(name, &val);
			tar = val.to_object();
			if (tar)	
			{
	...
	static lfl_string s_constructor("__constructor__");
