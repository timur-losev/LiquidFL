...
namespace gameswf
{
	...
	exported_module lfl_string	call_method(
		as_environment* env,
		as_object* this_ptr,
		const char* method_name,
	...
	struct event_id
	{
		...
		const lfl_string&	get_function_name() const;
	};
	...
	struct action_buffer
	{
		...
		static as_object* load_as_plugin(player* player,
			const lfl_string& classname, const array<as_value>& params);
