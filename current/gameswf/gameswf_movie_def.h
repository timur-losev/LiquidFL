...
//				printf("signal for frame %d\n", m_loading_frame);
		...
		virtual character_def*	get_exported_resource(const lfl_string& symbol) = 0;
		...
		virtual void	export_resource(const lfl_string& symbol, character_def* res) = 0;
		virtual void	add_import(const lfl_string& source_url, int id, const lfl_string& symbol_name) = 0;
	...
	struct import_info
	{
		...
		import_info(const lfl_string& source, int id, const lfl_string& symbol):
			m_source_url(source),
			m_character_id(id),
			m_symbol(symbol)
		{
	...
	struct movie_def_impl : public movie_definition_sub
	{
		...
		hash<int, lfl_string>	m_frame_label;	// this is't labeled_frame !!!
		...
		virtual void	export_resource(const lfl_string& symbol, character_def* res);
		virtual character_def*	get_exported_resource(const lfl_string& symbol);
		...
		virtual void	add_frame_label(int character_id, const lfl_string& class_name);
	...
	typedef void (*loader_function)(lfl_stream* input, int tag_type, movie_definition_sub* m);
