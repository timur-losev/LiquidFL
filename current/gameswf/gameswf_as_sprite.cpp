...
namespace gameswf
{
	...
	sprite_instance* sprite_getptr(const function_call& fn)
	{
...
//				UNUSED(sprite);
	...
	{ 
		...
		if (fn.nargs >= 3)
		{
			lfl_string id = fn.arg(0).to_string();	// the exported name (sprite_definition)
			lfl_string name = fn.arg(1).to_string();	// instance name
