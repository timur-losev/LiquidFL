...
struct video_handler_ogl : public gameswf::video_handler
{
		...
		const gameswf::matrix* m, const gameswf::rect* bounds, const gameswf::rgba& color)
	{
		...
		if (data)
		{
			int w2p = powerof2(width);
			int h2p = powerof2(height);
...
void bitmap_info_ogl::layout()
{
	if (m_texture_id == 0)
	{
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
#if	LFL_RENDER_USE_NPOTD_TEXTURES
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
#else	// LFL_RENDER_USE_NPOTD_TEXTURES
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
#endif	// LFL_RENDER_USE_NPOTD_TEXTURES
		...
		switch (m_suspended_image->m_type)
		{
			...
			{
#if	LFL_RENDER_USE_NPOTD_TEXTURES == 0
				int	w = powerof2(m_suspended_image->m_width);
				int	h = powerof2(m_suspended_image->m_height);
...
#else	// LFL_RENDER_USE_NPOTD_TEXTURES
				const int w = m_suspended_image->m_width;
				const int h = m_suspended_image->m_height;
#endif	// LFL_RENDER_USE_NPOTD_TEXTURES
				{
