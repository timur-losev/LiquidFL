...
namespace gameswf
{
	...
	{
#ifdef WIN32
	static lfl_string s_gameswf_version("WIN "__DATE__" "__TIME__);
#else
	static lfl_string s_gameswf_version("LINUX "__DATE__" "__TIME__);
#endif
	...
	{
		registered_type_node(const lfl_string& classname, gameswf_module_init type_init_func) :
			m_next(NULL),
		...
		lfl_string             m_classname;
		gameswf_module_init   m_type_init;
	};

	static registered_type_node* s_registered_types = NULL;

	void register_type_handler(const lfl_string& type_name, gameswf_module_init type_init_func )
	{
	...
	gameswf_module_init find_type_handler(const lfl_string& type_name)
	{
		registered_type_node *node = s_registered_types;
		while(node)
		{
	...
	as_standard_member	get_standard_member(const tu_stringi& name)
	{
		...
		{
#if _TU_USE_STL
            //s_standard_property_map.reserve(int(AS_STANDARD_MEMBER_COUNT));
#else
	...
	static const lfl_string	s_property_names[] =
	{
		lfl_string("_x"),
		lfl_string("_y"),
		lfl_string("_xscale"),
		lfl_string("_yscale"),
		lfl_string("_currentframe"),
		lfl_string("_totalframes"),
		lfl_string("_alpha"),
		lfl_string("_visible"),
		lfl_string("_width"),
		lfl_string("_height"),
		lfl_string("_rotation"),
		lfl_string("_target"),
		lfl_string("_framesloaded"),
		lfl_string("_name"),
		lfl_string("_droptarget"),
		lfl_string("_url"),
		lfl_string("_highquality"),
		lfl_string("_focusrect"),
		lfl_string("_soundbuftime"),
		lfl_string("mysteryquality"), //lfl_string("@@ mystery quality member"),  //this seems like a stupid bug to me . . . but I don't want it accessing the heap yet.
		lfl_string("_xmouse"),
		lfl_string("_ymouse"),
	};
	...
	void player::set_flash_vars(const lfl_string& param)
...
//		assert(m_current_root.get_ptr() != NULL);
	...
	movie_definition*	player::create_movie(const char* filename)
	{
		...
		if (m && s_use_cache_files)
		{
			...
			lfl_string	cache_filename(filename);
			cache_filename += ".gsc";
