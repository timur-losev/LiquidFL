...
//		virtual bool	has_looped() const { return m_has_looped; }
		...
		exported_module void	goto_frame(const lfl_string& target_frame);
		exported_module void	goto_frame(int target_frame_number);
		exported_module bool	goto_labeled_frame(const char* label);

		void	display();

		character*	add_display_object( Uint16 character_id, const lfl_string& name,
		...
		void	remove_display_object(const lfl_string& name);
		...
		character*	clone_display_object(const lfl_string& newname, int depth);
		...
		sprite_instance* attach_movie(const lfl_string& id, const lfl_string name, int depth);
		...
		virtual void dump(lfl_string& tabs);
