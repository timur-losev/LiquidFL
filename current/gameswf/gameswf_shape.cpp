...
//		memcpy(&m_coords[0], coords, coord_count * sizeof(coords[0]));
	...
	static void	read_fill_styles(array<fill_style>* styles, lfl_stream* in, int tag_type, movie_definition_sub* m)
	...
	static void	read_line_styles(array<line_style>* styles, lfl_stream* in, int tag_type, movie_definition_sub* m)
	...
	void	shape_character_def::read(lfl_stream* in, int tag_type, bool with_style, movie_definition_sub* m)
	{
		if (with_style)
		{
			m_bound.read(in);
