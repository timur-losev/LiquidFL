...
namespace gameswf
{
	exported_module void	as_object_addproperty(const function_call& fn);
	exported_module void	as_object_registerclass( const function_call& fn );
	exported_module void	as_object_hasownproperty(const function_call& fn);
	exported_module void	as_object_watch(const function_call& fn);
	exported_module void	as_object_unwatch(const function_call& fn);
	exported_module void	as_object_dump(const function_call& fn);
	exported_module void	as_global_object_ctor(const function_call& fn);

	// flash9
	exported_module void	as_object_add_event_listener(const function_call& fn);
	...
	struct as_object : public as_object_interface
	{
		...
		exported_module virtual bool watch(const lfl_string& name, as_function* callback, const as_value& user_data);
		exported_module virtual bool unwatch(const lfl_string& name);
		...
		exported_module virtual void dump(lfl_string& tabs);
