...
namespace gameswf
{
	...
	struct text_glyph_record
	{
		...
		void	read(lfl_stream* in, int glyph_count, int glyph_bits, int advance_bits)
		{
	...
	struct text_character_def : public character_def
	{
		...
		void	read(lfl_stream* in, int tag_type, movie_definition_sub* m);
		void	display(character* inst);
		void	csm_textsetting(lfl_stream* in, int tag_type);
	...
	struct edit_text_character : public character
	{
		...
		const lfl_string&	get_var_name() const;
		...
		void	format_plain_text(const lfl_string& text, text_glyph_record& rec);
	...
	private:
		...
		void	set_text(const lfl_string& new_text);
