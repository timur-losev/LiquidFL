...
namespace gameswf
{
    ...
    void metadata_info::read(lfl_stream* in, abc_def* abc)
    {
        ...
        int* keys = new int[m_item_count];
        //memset -> 0 is unnecessary here
        int i = 0;
        for (; i < m_item_count; ++i)
        {
            keys[i] = in->read_vu30();
        }
	...
	//	}
	void traits_info::read(lfl_stream* in, abc_def* abc)
	{
	...
	//	}
	void instance_info::read(lfl_stream* in, abc_def* abc)
	{
		m_abc = abc;
		m_name = in->read_vu30();
	...
	void class_info::read(lfl_stream* in, abc_def* abc)
	{
	...
	void script_info::read(lfl_stream* in, abc_def* abc)
	{
	...
	//	}
	void	abc_def::read(lfl_stream* in, movie_definition_sub* m)
	{
	...
	//	}
	void abc_def::read_cpool(lfl_stream* in)
	{
		int n;
	...
	as_function* abc_def::get_class_constructor(const lfl_string& name) const
	{
	...
	instance_info* abc_def::get_instance_info(const lfl_string& full_class_name) const
	{
		//TODO: implement namespace

		// find name
		lfl_string class_name = full_class_name;
		...
		{
			const lfl_string& name = get_multiname(m_instance[i]->m_name);
			if (class_name == name)
			{
	...
	class_info* abc_def::get_class_info(const lfl_string& full_class_name) const
	{
		//TODO: implement namespace
	...
	as_function* abc_def::get_script_function( const lfl_string & name ) const
	{
		if( name == "" )
