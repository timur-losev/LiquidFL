...
namespace gameswf
{
	...
	void	gradient_record::read(lfl_stream* in, int tag_type)
	{
		m_ratio = in->read_u8();
	...
	void	fill_style::read(lfl_stream* in, int tag_type, movie_definition_sub* md)
	{
		m_type = in->read_u8();
...
//			int gradient_count = in->read_uint(4);
	...
	void	line_style::read(lfl_stream* in, int tag_type, movie_definition_sub* m)
	{
		m_width = in->read_u16();
