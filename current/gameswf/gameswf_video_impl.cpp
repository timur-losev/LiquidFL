...
namespace gameswf
{
	...
	void video_stream_definition::read(lfl_stream* in, int tag, movie_definition* m)
	{
	...
	void attach_video(const function_call& fn)
	{
	...
	void clear_video_background(const function_call& fn)
	{
		video_stream_instance* video = cast_to<video_stream_instance>(fn.this_ptr);
		if (video && fn.nargs > 0)
		{
