...
#define COMPILE_DISASM 1
    ...
    struct inst_info_avm2
    {
        ...
        int process(const abc_def* def, const uint8 *args)
        {
            ...
            for( int i=0; i<m_arg_formats.size();++i)
            {
                ...
                case ARG_OFFSET:
                    {
                        // 24 bytes for offset value
                        // Timur: Offset can be negative. Fixed.
                        int8 s = *reinterpret_cast<const int8*>(&args[byte_count + 2]);
                        value = args[byte_count] | args[byte_count + 1] << 8 | s << 16; //sign extend the high byte
                        byte_count += 3;
                        log_msg( "\t\toffset: %i\n", value);
                    }
                    break;
                ...
                case ARG_OFFSETLIST:
                    {
                        ...
                        {
                            s = *reinterpret_cast<const int8*>(&args[byte_count + 2]); //sign extend the high byte

                            value = args[byte_count] | args[byte_count + 1] << 8 | s << 16;
                            log_msg("\t\toffset %i: %i\n", i, value);
                            byte_count +=3;
                        }
                    }
        ...
        // Disassemble one instruction to the log, AVM2
    {
        if (s_instr.size() == 0)
        {
            ...
            s_instr.add(0xAF, inst_info_avm2("greaterthan"));
            s_instr.add(0xB0, inst_info_avm2("greaterequals"));
        ...
        do
        {
            ...
            {
                printf("%d:	%s. Code[0x%02X]\n",ip , ii.m_instruction, opcode);
                if( ii.has_argument() )
                {
