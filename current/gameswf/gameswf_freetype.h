...
namespace gameswf
{
	...
	typedef bool (*font_loader_pfn)(const char* , lfl_string& , bool , bool );//Timur - callback to load font

	exported_module void register_font_loader_clb(font_loader_pfn clb);
	...
	struct glyph_freetype_provider : public glyph_provider
	{
		...
		virtual bitmap_info* get_char_image(character_def* shape_glyph, Uint16 code, 
			const lfl_string& fontname, bool is_bold, bool is_italic, int fontsize,
			rect* bounds, float* advance);

	private:
		
		face_entity* get_face_entity(const lfl_string& fontname,
