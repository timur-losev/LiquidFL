...
namespace gameswf
{
	...
	bool get_fontfile(const char* font_name, lfl_string& file_name, bool is_bold, bool is_italic)
	// gets font file name by font name
	{
		...
		lfl_string windir(getenv("WINDIR"));
		...
		{ 
			...
			{ 
				...
				{
					file_name = windir + lfl_string("\\Fonts\\") + (char*) szValueData;
					RegCloseKey(hKey);
					return true;
				}
			} 
	...
	static font_loader_pfn s_get_font_file_clb = get_fontfile;
	
	void register_font_loader_clb(font_loader_pfn clb)
	{
		s_get_font_file_clb = clb;
	}
	...
	bitmap_info* glyph_freetype_provider::get_char_image(character_def* shape_glyph, Uint16 code, 
			const lfl_string& fontname, bool is_bold,
	...
	face_entity* glyph_freetype_provider::get_face_entity(const lfl_string& fontname, bool is_bold, bool is_italic)
	{
		// form hash key
		lfl_string key = fontname;
		if (is_bold)
		{
			key += "B";
		}
		...
		lfl_string font_filename;
		//if (get_fontfile(fontname, font_filename, is_bold, is_italic) == false)
		if(s_get_font_file_clb(fontname, font_filename, is_bold, is_italic) == false)
		{
