...
namespace gameswf
{
	...
	void	as_global_textfield_ctor(const function_call& fn)
...
//				* pixel_scale;
	...
	void	text_character_def::read(lfl_stream* in, int tag_type, movie_definition_sub* m)
	{
		assert(m != NULL);
	...
	void	text_character_def::csm_textsetting(lfl_stream* in, int tag_type)
	{
	...
	void	edit_text_character_def::read(lfl_stream* in, int tag_type, movie_definition_sub* m)
	{
		assert(m != NULL);
		assert(tag_type == 37);
	...
	void	edit_text_character_def::csm_textsetting(lfl_stream* in, int tag_type)
	{
		assert(tag_type == 74);	// Flash 8
		m_use_flashtype = in->read_uint(2) == 0 ? false : true;
	...
	void	set_textformat(const function_call& fn)
	{
	...
	const lfl_string&	edit_text_character::get_var_name() const
	{
	...
	void	edit_text_character::set_text(const lfl_string& new_text)
	...
	void	edit_text_character::set_text_value(const lfl_string& new_text)
	...
	const char*	edit_text_character::html_font(const char* p, text_glyph_record& rec)
	{
		...
		while (true)
		{
			...
			if (strncmp(p, "letterSpacing=", 14) == 0)
			{
				...
				const char* end = strchr(p, '"');
				int len = int(end - p);
				lfl_string letterSpacing(p, len);
				p += len + 1;
			}
			else
...
//		printf("%s\n", m_text.c_str());
	...
	void	edit_text_character::format_plain_text(const lfl_string& text, text_glyph_record&	rec)
	{
		...
		const lfl_string* textPtrToUse = &text;
		lfl_string passwordText;
		if (m_password)
		{
			// nice big hack.
			passwordText = text;
