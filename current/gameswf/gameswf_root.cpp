...
namespace gameswf
{
	...
	lfl_string root::call_method(const char* method_name, as_value * arguments, int argument_count )
	{
		assert(m_movie != NULL);
	...
	void root::set_flash_vars(const lfl_string& vars)
	{
		for (const char* word = vars.c_str(); *word; )
		{
			...
			lfl_string varname(word, int(delimiter - word));
			...
			lfl_string value(word, int(delimiter - word));
