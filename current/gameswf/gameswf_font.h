...
namespace gameswf
{
	...
	struct font : public character_def
	{
		...
		void	read_font_info(lfl_stream* in, int tag_type);
		void	read_font_alignzones(lfl_stream* in, int tag_type);

		void	output_cached_data(tu_file* out, const cache_options& options);
		void	input_cached_data(tu_file* in);

		const lfl_string& get_name() const { return m_fontname; }
		void	set_name(const lfl_string& name) { m_fontname = name; }
	...
	private:
		void	read_code_table(lfl_stream* in);

		array<gc_ptr<shape_character_def> >	m_glyphs;
		lfl_string	m_fontname;
