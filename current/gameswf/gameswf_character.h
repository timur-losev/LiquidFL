...
namespace gameswf
{
	...
	struct character_def : public as_object_interface
	{
		...
		virtual void	csm_textsetting(lfl_stream* in, int tag_type) { assert(0); };
	...
	struct character : public as_object
	{
		...
		virtual character_def*	find_exported_resource(const lfl_string& symbol)
		{
			character* ch = get_parent();
			if (ch)
			{
...
//#endif	// not __GNUC__
		...
		virtual lfl_string	call_method(const char* method_name, as_value * arguments, int argument_count )
			// Override this if you implement call_method.
		{
			assert(0);
			return lfl_string();
		...
		virtual character*	add_display_object( Uint16 character_id, const lfl_string& name, const array<swf_event*>& event_handlers, int depth, bool replace_if_depth_is_occupied, const cxform& color_transform, const matrix& mat, float ratio, Uint16 clip_depth, Uint8 blend_mode)
		{
			return NULL;
		}
		...
		virtual character*	clone_display_object(const lfl_string& newname, int depth)
		{
			assert(0);
			return NULL; 
		}

		virtual void	remove_display_object(const lfl_string& name) { assert(0); }
...
//		virtual character*	get_root_interface() { return NULL; }
		...
		void	set_name(const lfl_string& name) { m_name = name; }
		const lfl_string&	get_name() const { return m_name; }
