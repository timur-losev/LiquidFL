...
#include "gameswf/lfl_actions_def.h"
...
namespace gameswf
{
	...
	void	as_global_trace(const function_call& fn);
	...
	// then arg1 is at env->bottom(7), arg2 is at env->bottom(6), etc.
	{
		...
		if (func)
		{
			...
			(*func)(function_call(&val, this_ptr, env, nargs, first_arg_bottom_index));
		}
		else
		{
	...
	void	action_buffer::read(lfl_stream* in)
	{
		// Read action bytes.
		for (;;)
		{
...
//		env->set_local_frame_top(local_stack_top);
	...
	as_object* action_buffer::load_as_plugin(player* player, const lfl_string& classname,
...
#define CHECK_STACK(n)							\
		...
		for (int pc = start_pc; pc < stop_pc && !found_error; )
		{
			...
			if ((action_id & 0x80) == 0)
			{
				...
				switch (action_id)
				{
					...
					case ACTION_EndOfActions:	// end of actions.
						return;

					case ACTION_NextFrame:	// next frame.
					{
					...
					case ACTION_PrevFrame:	// prev frame.
					{
						if (env->get_target() != NULL)
						{
							env->get_target()->goto_frame(env->get_target()->get_current_frame() - 1);
					...
					case ACTION_ActionPlay:	// action play
					{
						if (env->get_target() != NULL)
						{
					...
					case ACTION_ActionStop:	// action stop
					{
						if (env->get_target() != NULL)
						{
					...
					case ACTION_ToggleQuality:	// toggle quality
					{
						// @@ TODO
						log_error("todo opcode: %02X\n", action_id);
					} break;

					case ACTION_StopSounds:	// stop sounds
					{
					...
					case ACTION_Sub:	// subtract
					{
						env->top(1) -= env->top(0).to_number();
						env->drop(1);
					} break;

					case ACTION_Multiply:	// multiply
					{
						env->top(1) *= env->top(0).to_number();
						env->drop(1);
					} break;

					case ACTION_Divide:	// divide
					{
						env->top(1) /= env->top(0).to_number();
						env->drop(1);
					} break;

					case ACTION_Equal:	// equal
					{
						env->top(1).set_bool(env->top(1) == env->top(0));
						env->drop(1);
					} break;

					case ACTION_LessThan:	// less than
					{
						env->top(1).set_bool(env->top(1) < env->top(0).to_number());
						env->drop(1);
					} break;

					case ACTION_LogicalAnd:	// logical and
					{
						env->top(1).set_bool(env->top(1).to_bool() && env->top(0).to_bool());
						env->drop(1);
					} break;

					case ACTION_LogicalOr:	// logical or
					{
						env->top(1).set_bool(env->top(1).to_bool() || env->top(0).to_bool());
						env->drop(1);
					} break;

					case ACTION_LogicalNot:	// logical not
					{
					...
					case ACTION_SubString:	// substring
					{
						...
						// of as_value and lfl_string...
						lfl_string	new_string = str.c_str() + base;
					...
					case ACTION_SetTarget:	// set target expression
					{
						// Flash doc says:
					...
					case ACTION_SetProperty:	// set property
					{
					...
					case ACTION_DuplicateMovieClip:
					...
					case ACTION_RemoveMovieClip:	// remove clip
					{
					...
					case ACTION_Trace:	// trace
					{
						// Log the stack val.
						as_global_trace(function_call(&env->top(0), as_value(), env, 1, env->get_top_index()));
						env->drop(1);
						break;
					}

					case ACTION_StartDragMovieClip:	// start drag movie
					{
					...
					case ACTION_StopDragMovieClip:	// stop drag movie
					{
					...
					case ACTION_CastObject:	// cast_object
					{
					...
					case ACTION_Implements:	// implements
					{
					...
					case ACTION_MBStringLength:	// mb length
					{
						// @@ TODO
						log_error("todo opcode: %02X\n", action_id);
					} break;

					case ACTION_CharToAscii:	// ord
					{
						// ASCII code of first character
						env->top(0).set_int(env->top(0).to_string()[0]);
					} break;

					case ACTION_AsciiToChar:	// chr
					{
						char	buf[2];
					...
					case ACTION_MBCharToAscii:	// ActionMBCharToAscii
					{
						/*	iOrange - @@ TODO
					...
					case ACTION_MBAsciiToChar:	// ActionMBAsciiToChar
					{
						// @@ TODO
						log_error("todo opcode: %02X\n", action_id);
					} break;
					...
					case ACTION_CallFunction:	// call function
					{
						as_value	function;
					...
					case ACTION_New:	// new
					{
						...
						if (as_c_function* c_constructor = cast_to<as_c_function>(constructor.to_object()))
						{
							...
							(*c_constructor)(function_call(&new_obj, as_value(), env, nargs, env->get_top_index()));
						}
					...
					case ACTION_InitArray:	// init array
					{
 						// Call the array constructor
 						as_value	result;
 						as_global_array_ctor(function_call(&result, as_value(), env, -1, -1));
 						env->push(result);
					} break;

					case ACTION_DeclareObject:	// declare object
					{
	...
	//						obj->builtin_member(env->top(1).to_tu_string(), env->top(0));
					...
					case ACTION_TypeOf:	// type of
					{
						env->top(0).set_string(env->top(0).type_of());
					} break;

					case ACTION_GetTarget:	// get target
					{
						// @@ TODO
						log_error("todo opcode: %02X\n", action_id);
					} break;
					...
					case ACTION_Add_Typed:	// add_t (typed)
					{
					...
					case ACTION_Equal_Typed:	// equal (typed)
					{
					...
					case ACTION_ToNumber:	// to number
					{
					...
					case ACTION_GetMember:	// get member
					{
						CHECK_STACK(2);
						// keep alive 'obj'
						...
						else
						{
							...
							if (obj->get_member(env->top(0).to_tu_string(), &(env->top(1))) == false)
							{
								...
								if (obj->get_member("__resolve", &val))
								{
									...
									if (resolve)
									{
										(*resolve)(function_call(&val, obj.get(), env, 1, env->get_top_index()));
										env->top(1) = val;
									}
								}
							}
					...
					case ACTION_SetMember:	// set member
					{
						CHECK_STACK(3);
	...
	//								env->top(2).to_tu_string().c_str(),
					...
					case ACTION_Increment:	// increment
					{
						CHECK_STACK(1);
						env->top(0) += 1;
					} break;

					case ACTION_Decrement:	// decrement
					{
						CHECK_STACK(1);
						env->top(0) -= 1;
					} break;

					case ACTION_CallMethod:	// call method
					{
						CHECK_STACK(3);
	...
	//#endif
					...
					case ACTION_NewMethod:	// new method
					{
	...
	//								prototype.to_object()->set_member("__constructor__", func);
					...
					case ACTION_InstanceOf:	// instance of
					{
						CHECK_STACK(2);
					...
					case ACTION_EnumerateObject:	// enumerate object
					{
						CHECK_STACK(1);
					...
					case ACTION_BitwiseAnd:	// bitwise and
					{
						CHECK_STACK(2);
					...
					case ACTION_BitwiseOr:	// bitwise or
					{
						CHECK_STACK(2);
						env->top(1) |= env->top(0).to_int();
						env->drop(1);
					...
					case ACTION_BitwiseXor:	// bitwise xor
					{
						CHECK_STACK(2);
						env->top(1) ^= env->top(0).to_int();
						env->drop(1);
					...
					case ACTION_BitwiseShiftLeft:	// shift left
					{
						CHECK_STACK(2);
					...
					case ACTION_BitwiseShiftRight_signed:	// shift right (signed)
					{
						CHECK_STACK(2);
					...
					case ACTION_BitwiseShiftRight_unsigned:	// shift right (unsigned)
					{
						CHECK_STACK(2);
					...
					case ACTION_StrictEqual:	// strict equal
					{
						CHECK_STACK(2);
						env->top(1).set_bool(env->top(1) == env->top(0));
						env->drop(1);
					...
					case ACTION_GreaterThan_Typed:	// gt (typed)
					{
						CHECK_STACK(2);
						if (env->top(1).is_string())
							env->top(1).set_bool(env->top(1).to_tu_string() > env->top(0).to_tu_string());
				...
				}	// end of switch (action_id)

				pc++;	// advance to next action.
			}	// end of if ((action_id & 0x80) == 0)
			else
			{
				IF_VERBOSE_ACTION(log_msg("EX:\t"); log_disasm(&buffer[pc]));
				...
				switch (action_id)
				{
					...
					case 0x8E:	// function2
					{
						...
						lfl_string	name = (const char*) &buffer[i];
						i += name.length() + 1;
	...
	static lfl_string	s_function_names[event_id::EVENT_COUNT] =
	{
		"INVALID",		 // INVALID
		"onPress",		 // PRESS
		"onRelease",		 // RELEASE
	...
	const lfl_string&	event_id::get_function_name() const
	{
