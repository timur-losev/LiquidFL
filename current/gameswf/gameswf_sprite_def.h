...
namespace gameswf
{
	...
	struct sprite_definition : public movie_definition_sub
	{
		...
		virtual void	export_resource(const lfl_string& symbol, character_def* res) { log_error("can't export from sprite\n"); }
		virtual character_def*	get_exported_resource(const lfl_string& sym) { return m_movie_def->get_exported_resource(sym); }
		virtual void	add_import(const lfl_string& source_url, int id, const lfl_string& symbol) { assert(0); }
