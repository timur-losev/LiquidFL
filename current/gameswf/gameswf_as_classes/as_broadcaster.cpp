...
namespace gameswf
{
	...
	void	as_broadcast_addlistener(const function_call& fn)
	...
	// (listenerObj:Object) : Boolean
	void	as_broadcast_removelistener(const function_call& fn)
	...
	void	as_broadcast_sendmessage(const function_call& fn)
	...
	void	as_broadcaster_initialize(const function_call& fn)
	{
		if (fn.nargs == 1)
		{
	...
	void	as_listener::broadcast(const function_call& fn)
	{
		assert(fn.env);
		...
		m_listeners.notify(event_name, 
			function_call(NULL, 0, fn.env, fn.nargs - 1, fn.env->get_top_index()));
		...
		while (m_suspended_event.size() > 0)
		{
			...
			m_listeners.notify(event_name, 
				function_call(NULL, 0, fn.env, arg.size() - 1, fn.env->get_top_index()));
