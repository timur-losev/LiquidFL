...
namespace gameswf
{
	...
	void	as_global_point_ctor(const function_call& fn)
	{
		gc_ptr<as_point>	obj;
		float x = 0.0f;
		float y = 0.0f;
	...
	void	as_point_add(const function_call& fn)
	{
		if (fn.nargs < 1)
		{
			return;
	...
	void	as_point_subtract(const function_call& fn)
	{
		if (fn.nargs < 1)
		{
			return;
	...
	void	as_point_normalize(const function_call& fn)
	{
		if (fn.nargs < 1)
		{
			return;
