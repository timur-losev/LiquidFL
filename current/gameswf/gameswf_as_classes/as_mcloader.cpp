...
namespace gameswf
{
	...
	void	as_mcloader_addlistener(const function_call& fn)
	{
	...
	void	as_mcloader_removelistener(const function_call& fn)
	{
		as_mcloader* mcl = cast_to<as_mcloader>(fn.this_ptr);
		assert(mcl);
	...
	void	as_mcloader_loadclip(const function_call& fn)
	{
		as_mcloader* mcl = cast_to<as_mcloader>(fn.this_ptr);
		assert(mcl);
		...
		if (fn.nargs == 2)
		{
			...
			lfl_string infile = get_full_url(fn.get_player()->get_workdir(), fn.arg(0).to_string());
	...
	void	as_mcloader_unloadclip(const function_call& fn)
	{
		if (fn.nargs == 1)
		{
	...
	void	as_mcloader_getprogress(const function_call& fn)
	{
		if (fn.nargs == 1)
		{
	...
	void	as_global_mcloader_ctor(const function_call& fn)
