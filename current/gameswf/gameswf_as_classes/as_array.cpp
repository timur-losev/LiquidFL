...
namespace gameswf
{
	...
	void	as_array_join(const function_call& fn)
	{
	...
	void	as_array_tostring(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a)
		{
	...
	void	as_array_push(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a)
		{
	...
	void	as_array_shift(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a && a->size() > 0)
		{
	...
	void	as_array_pop(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a && a->size() > 0)
		{
	...
	void	as_array_splice(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a && fn.nargs >= 1)
		{
	...
	void	as_array_concat(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a)
		{
	...
	void	as_array_slice(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a)
		{
	...
	void	as_array_unshift(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a)
		{
			for (int i = fn.nargs - 1; i >= 0; i--)
			{
				...
				if (arg)
				{
					...
					{
						as_value val;
						if (arg->get_member(lfl_string(j), &val))
						{
							a->insert(0, val);
						}
					}
	...
	void	as_array_sort(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a)
		{
	...
	void	as_array_length(const function_call& fn)
	{
		as_array* a = cast_to<as_array>(fn.this_ptr);
		if (a)
		{
	...
	void	as_global_array_ctor(const function_call& fn)
