...
namespace gameswf
{
	...
	// Color(target:Object)
	void	as_global_color_ctor(const function_call& fn)
	{
		if (fn.nargs == 1)
		{
	...
	void	as_color_getRGB(const function_call& fn)
	{
	...
	void	as_color_setRGB(const function_call& fn)
	{
		if (fn.nargs < 1)
		{
			return;
	...
	void	as_color_gettransform(const function_call& fn)
	{
		as_color* obj = cast_to<as_color>(fn.this_ptr);
		if (obj == NULL)
		{
	...
	void	as_color_settransform(const function_call& fn)
	{
		if (fn.nargs < 1)
		{
			return;
