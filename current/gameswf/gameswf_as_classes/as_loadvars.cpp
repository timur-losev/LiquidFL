...
namespace gameswf
{
	...
	void	as_loadvars_addrequestheader(const function_call& fn)
	{
		if( fn.nargs == 1 )
		{
	...
	void	as_loadvars_decode(const function_call& fn)
	{
		if( fn.nargs < 1 )
		{
			return;
	...
	void	as_loadvars_getbytesloaded(const function_call& fn)
	{
		assert( 0 && "todo" );
	}

	//getBytesTotal() : Number
	void	as_loadvars_getbytestotal(const function_call& fn)
	{
		assert( 0 && "todo" );
	}

	//load(url:String) : Boolean
	void	as_loadvars_load(const function_call& fn)
	{
	...
	void	as_loadvars_send(const function_call& fn)
	{
		assert( 0 && "todo" );
	}

	//sendAndLoad(url:String, target:Object, [method:String]) : Boolean
	void	as_loadvars_sendandload(const function_call& fn)
	{
		if (fn.nargs < 2)
		{
			fn.result->set_bool(false);
	...
	//toString() : String
	void	as_loadvars_tostring(const function_call& fn)
	{
		as_loadvars* loadvars = cast_to<as_loadvars>(fn.this_ptr);
	...
	bool	as_loadvars::send_and_load(const char * c_url, as_object * target, const lfl_string & method)
	{
		lfl_string host, uri;
		...
		lfl_string request_string = create_request( method, uri, true );
	...
	void	as_loadvars::add_header(const lfl_string& name, const lfl_string& value)
	{
		m_headers.set(name, value);
	}

	lfl_string	as_loadvars::override_to_string()
	{
		lfl_string information;
		//create information in the form of name=value (urlencoded)
		string_hash<lfl_string>::iterator it = m_received_values.begin();
	...
	void	as_loadvars::decode(const lfl_string& query_string)
	{
		char *start, *end;
		...
		while( start < query_string.c_str() + query_string.size() )
		{
			...
			*end=0;
			...
			lfl_string name = lfl_string(start, int(after_name - start));
			lfl_string value = lfl_string( after_name + 1 ); //Skip the "="
	...
	lfl_string 	as_loadvars::create_header()
	{
		lfl_string header;

		// Process header
		for( string_hash<lfl_string>::iterator it2 = m_headers.begin(); it2 != m_headers.end(); ++it2 )
		{
	...
	void	as_loadvars::parse_request(const lfl_string& line, request_data& request)
	{
	...
	void	as_loadvars::parse_header(const lfl_string& line, request_data& request)
	{
		if( line == "\r\n" )
		{
	...
	void	as_loadvars::parse_content(const lfl_string& line, request_data& request)
	{
	...
	bool as_loadvars::parse_url(const char* c_url, lfl_string& host, lfl_string& uri)
	{
	...
	{
		for (string_hash<lfl_string>::iterator it = m_received_values.begin(); it != m_received_values.end(); ++it)
		{
