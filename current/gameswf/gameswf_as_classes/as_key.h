...
namespace gameswf
{
	void	key_add_listener(const function_call& fn);
	...
	void	key_get_ascii(const function_call& fn);
	// Return the ascii value of the last key pressed.

	void	key_get_code(const function_call& fn);
	// Returns the keycode of the last key pressed.

	void	key_is_down(const function_call& fn);
	// Return true if the specified (first arg keycode) key is pressed.

	void	key_is_toggled(const function_call& fn);
	// Given the keycode of NUM_LOCK or CAPSLOCK, returns true if
	// the associated state is on.

	void	key_remove_listener(const function_call& fn);
