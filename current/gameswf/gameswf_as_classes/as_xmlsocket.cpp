...
namespace gameswf
{
	...
	void	as_xmlsock_connect(const function_call& fn)
	{
		if (fn.nargs != 2)
		{
	...
	void	as_xmlsock_close(const function_call& fn)
	{
		as_xmlsock* xmls = cast_to<as_xmlsock>(fn.this_ptr);
		assert(xmls);
	...
	void	as_xmlsock_send(const function_call& fn)
	{
		if (fn.nargs != 1)
		{
			return;
	...
	void	as_global_xmlsock_ctor(const function_call& fn)
