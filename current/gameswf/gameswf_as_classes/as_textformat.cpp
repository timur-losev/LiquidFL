...
namespace gameswf
{
	...
	static lfl_string as_global_textformat_ctor_paramnames[] =
	{"font", "size", "color", "bold", "italic", "underline", "url", "target",
	 "align", "leftMargin", "rightMargin", "indent", "leading"};

	void	as_global_textformat_ctor(const function_call& fn)
	{
		gc_ptr<as_textformat>	obj = new as_textformat(fn.get_player());

		assert(unsigned(fn.nargs) <= LFL_ARRAYSIZE(as_global_textformat_ctor_paramnames));
