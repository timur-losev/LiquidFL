...
namespace gameswf
{
	...
	void	as_matrix_translate(const function_call& fn)
	{
		if (fn.nargs < 2)
		{
			return;
	...
	void	as_matrix_rotate(const function_call& fn)
	{
		if (fn.nargs < 1)
		{
			return;
	...
	void	as_matrix_scale(const function_call& fn)
	{
		if (fn.nargs < 2)
		{
			return;
	...
	void	as_matrix_concat(const function_call& fn)
	{
		if (fn.nargs < 1)
		{
			return;
	...
	void as_matrix_clone(const function_call& fn)
	{
		as_matrix* matrix = cast_to<as_matrix>(fn.this_ptr);
	...
	void as_matrix_invert(const function_call& fn)
	{
		as_matrix* matrix = cast_to<as_matrix>(fn.this_ptr);
	...
	void as_matrix_transformPoint(const function_call& fn)
	{
		if (fn.nargs < 1)
		{
			return;
