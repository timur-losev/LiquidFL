...
namespace gameswf
{
	...
	void as_global_parse_float(const function_call& fn)
	{
		if (fn.nargs > 0)  
		{
			double res;
	...
	void as_global_parse_int(const function_call& fn)
	{
		if (fn.nargs > 0)
		{
			int res;
	...
	void as_global_isnan(const function_call& fn)
	{
		if (fn.nargs == 1)  
		{
	...
	// Number(num:Object)
	void	as_global_number_ctor(const function_call& fn)
	{
		if (fn.nargs == 1)
		{
	...
	void	as_number_to_string(const function_call& fn)
	{
	...
	void	as_number_valueof(const function_call& fn)
	{
		assert(0);	//TODO
	}
