...
static bool font_loader_callback(const char* font_name, lfl_string& file_name, bool is_bold, bool is_italic)
{
	if(font_name == NULL) 
	{
		message_log("Error: font_loader_callback >> font_name is null.");
		return false;
	}
...
// For handling notification callbacks from ActionScript.
{
	...
	if (stricmp(command, "notify_keypress") == 0)
	{
		...
		for (int i = 0, n = static_cast<int>(strlen(args)); i < n; i++)
		{
...
int	main(int argc, char *argv[])
{
	...
	{	// for testing memory leaks
		...
		// workdir is used when LoadMovie("myfile.swf", _root) is called
		{
			...
			int len = static_cast<int>(ptr - infile + 1);
			if (len > 0)
			{
				lfl_string workdir(infile, len);
		...
		gameswf::register_font_loader_clb(font_loader_callback);
