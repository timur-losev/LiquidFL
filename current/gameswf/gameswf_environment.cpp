...
namespace gameswf
{
	...
	// static
	lfl_string get_full_url(const lfl_string& workdir, const char* url)
	{
		 lfl_string fn;
	...
	character* as_environment::load_file(const char* url, const as_value& target_value, int method)
	{
		...
		// is path relative ?
		lfl_string file_name = get_full_url(get_player()->get_workdir(), url);
	...
	as_value	as_environment::get_variable(const lfl_string& varname, const array<with_stack_entry>& with_stack) const
	...
	void	as_environment::set_local(const lfl_string& varname, const as_value& val)
	...
	void	as_environment::add_local(const lfl_string& varname, const as_value& val)
	...
	void	as_environment::declare_local(const lfl_string& varname)
	...
	int	as_environment::find_local(const lfl_string& varname, bool ignore_barrier) const
	...
	bool	as_environment::parse_path(const lfl_string& var_path, lfl_string* path, lfl_string* var)
	...
	player* function_call::get_player() const
	{
		assert(env);
		return env->get_player();
	}

	root* function_call::get_root() const
	{
		assert(env);
