...
namespace gameswf
{
	...
	// Return true if we have any mouse event handlers.
	{
		...
		if (is_enabled())
		{
			for (size_t i = 0; i < LFL_ARRAYSIZE(FN_NAMES); i++)
			{
				as_value dummy;
	...
	void sprite_instance::goto_frame(const lfl_string& target_frame)
	{
	...
	character* sprite_instance::add_display_object( Uint16 character_id, const lfl_string& name,
...
//		ch->on_event(event_id::CONSTRUCT);	// isn't tested
	...
	character* sprite_instance::clone_display_object(const lfl_string& newname, int depth)
	...
	void sprite_instance::remove_display_object(const lfl_string& name)
	...
	lfl_string sprite_instance::call_method(const char* method_name, as_value * arguments, int argument_count )
	{
	...
	void sprite_instance::attach_display_callback(const char* path_to_object, void (*callback)(void*), void* user_ptr)
	{
		...
		as_value	obj = m_as_environment.get_variable(lfl_string(path_to_object), dummy);
...
//		visited_objects->set(this, true);
	...
	sprite_instance* sprite_instance::attach_movie(const lfl_string& id, 
		const lfl_string name, 
		int depth)
	{
	...
	void sprite_instance::dump(lfl_string& tabs)
	{
		tabs += "  ";
	...
	character_def* sprite_instance::find_exported_resource(const lfl_string& symbol)
	{
	...
	void sprite_instance::enumerate(as_environment* env)
	{
		...
		for (int i = 0, n = m_display_list.size(); i<n; i++)
		{
			...
			if (ch)
			{
				const lfl_string& name = ch->get_name();
				if (name.size() > 0)
				{
					env->push(name);
				}
